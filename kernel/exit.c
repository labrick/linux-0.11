...
#include <errno.h>			// 错误号头文件。包含系统中各种错误号(Linus从minix中引进的)
...
int sys_pause(void);		// 把进程置为睡眠状态，直到收到信号(kernel/sched.c 114)
int sys_close(int fd);		// 关闭指定文件的系统调用(fs/open.c 192)

//// 释放指定进程占用的任务槽及其任务数据结构占用的内存页面
// 参数p是任务数据结构指针。该函数在后面的sys_kill()和sys_waitpid()函数中被调用。
// 扫描任务指针数组表task[]以寻找指定的任务。如果找到，则首先清空该任务槽，然后释放
// 该任务数据结构所占用的内存页面，最后执行调度函数并在返回时立即退出。如果在任务
// 数组表中没有找到指定任务对应的项，则内核panic :-)
void release(struct task_struct * p)
{
	int i;

	if (!p)						// 如果进程数据结构指针是NULL，则什么都不做，退出
		return;
	for (i=1 ; i<NR_TASKS ; i++)	// 扫描任务数组，寻找指定任务
		if (task[i]==p) { task[i]=NULL;			// 置空该任务项并释放相关内存页
			free_page((long)p);		// 仅仅释放一页的内存？？？
			schedule();				// 重新调度(似乎没有必要，还没理解调度的意义)
			return;
		}
...
// 参数：sig - 信号值；p - 指定任务的指针；priv - 强制发送信号的标志。即不需要考虑
// 进程用户属性或级别而能发送信号的权利。该函数首先判断参数的正确性，然后判断条件
// 是否满足。如果满足就向指定进程发送信号sig并退出，否则返回未许可错误号
static inline int send_sig(long sig,struct task_struct * p,int priv)
{
// 若信号不正确或任务指针为空则出错退出
	if (!p || sig<1 || sig>32)
		return -EINVAL;
// 如果强制发送标志置位，或者当前进程的有效用户标识符(euid)就是指定进程的euid(也就
// 是自己)，或者当前进程是超级用户，则向进程p发送信号sig，即在进程p位图中添加该信号，
// 否则出错退出。其中suser()定义为(current->euid==0)，用于判断是否是超级用户
...
// 如果pid=0，那么信号就会发送给当前进程的进程组中的所有进程；
// 如果pid=-1，则信号sig就会发送给除第一个进程(初始进程init)外的所有进程；
// 如果pid<-1，则信号sig就发送给进程组-pid的所有进程；
...
// 如果没有找到父进程，则自己释放。但根据POSIX.1要求，若父进程已先行终止，则子进程
// 应该被初始进程1收容。
...
//// 程序退出函数。在下面137行处的系统调用处理函数sys_exit()中被调用。
...
// 位置(current->ldt[2]给出进程代码段描述符的位置)；get_limit()中的0x0f是进程代码
// 段的选择符(0x17是进程数据段的选择符)。即在取段基地址时使用该段的描述符所处地址作
// 为参数，取段长度时使用该段的选择符作为参数。free_page_tables()函数位于mm/memory.c
// 文件的105行，get_base()和get_limit()宏位于include/linux/sched.h头文件的213行处
...
// 如果该子进程已经处于僵死(ZOMBIE)状态，则向进程1发送子进程终止信号SIGCHLD
	for (i=0 ; i<NR_TASKS ; i++)
		...
		/* assumption task[1] is always init */ /* 这里假设task[1]肯定是进程init */
...
// 关闭当前进程打开着的所有文件。
	for (i=0 ; i<NR_OPEN ; i++)
		if (current->filp[i])
			sys_close(i);
// 对当前进程的工作目录pwd、根目录root以及执行程序文件的i节点进程同步操作，放回各个
// i节点并分别置空(释放)，文件部分还没看，这部分可以回过头来继续???????
	iput(current->pwd);
	current->pwd=NULL;
	iput(current->root);
	current->root=NULL;
...
// volatile，就可以告诉gcc编译器本函数不会返回的特殊情况。这样可让gcc产生更好一些
// 的代码，并且可以不用再写这条return语句也不会产生假警告信息。
	return (-1);	/* just to suppress warnings */	/* 仅用于去掉警告信息 */
}

//// 系统调用exit()。终止进程
// 参数error_code是用户程序提供的退出状态信息，只有低字节有效。把error_code左移8
// 比特是wait()或waitpid()函数的要求。低字节中将用来保存wait()的状态信息。例如，
// 如果进程处于短暂状态(TASK_STOPPED)，那么其低字节就等于0x7f。参见sys/wait.h文件
// 第13--18。wait()或waitpid()利用这些宏就可以取得子进程的退出状态码或子进程终止
// 的原因(信号)。
...
//// 系统调用waitpid()。挂起当前进程，直到pid指定的子进程退出(终止)或者收到要求
// 终止该进程的信号，或者是需要调用一个信号句柄(信号处理程序)。如果pid所指的子进程
// 早已退出(已成所谓的僵死进程)，则本调用将立刻返回。子进程使用的所有资源将释放。
...
// 如果前3个对pid的判断都不符合，则表示当前进程正在等待其任何子进程，也即pid=-1的
// 情况。此时所选择到的进程p或者是其进程号等于指定pid，或者是当前进程组中的任何
// 子进程，或者是进程号等于指定pid绝对值的子进程，或者是任何子进程(此时指定的pid
// 等于-1)。接下来根据这个子进程p所处的状态来处理
		switch ((*p)->state) {
// 子进程p处于停止状态时，如果此时WUNTRACED标志没有置位，表示程序无须立刻返回，
// 于是继续扫描处理其他程序。如果WUNTRACED置位，则把状态信息0x7f放入*stat_addr，
...
// (父进程)中，然后取出子进程的pid和退出码，并释放该子进程。最后返回子进程的退出码
// 和pid
			case TASK_ZOMBIE:
...
// 在上面对任何数组扫描结束后，如果flag被置位，说明有符合等待要求的子进程并没有处
...
// 函数等待子进程。
	if (flag) {
		if (options & WNOHANG)				// 若options = WNOHANG，则立刻返回
			return 0;
