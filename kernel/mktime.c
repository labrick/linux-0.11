...
// 由于UNIX计年份y是从1970年算起。到1972年就是一个闰年，因此过3年(71,72,73)就是
// 第1个闰年，这样从1970年开始的闰年数计算方法就应该是1+(y-3)/4，即为(y+1)/4。
// res = 这些年经过的秒数时间 + 每个闰年时多1天的秒数时间 + 当年到当月时的秒数。
// 另外，month[]数组中已经在2月份的天数中包含进了闰年时的天数，即2月份天数多算了1天。
// 因此，若当年不是闰年并且当前月份大于2月份的话，我们就要减去这天。因为从70年开始
// 算起，所以当年是闰年的判断方法是(y+2)能被4除尽。若不能除尽(有余数)就不是闰年。
	year = tm->tm_year - 70;
