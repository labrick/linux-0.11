...
#include <asm/segment.h>	// 段操作文件。定义了有关段寄存器操作的嵌入式汇编函数
...
// 首先验证信号值是否在有效范围(1--32)内，并且不得是信号SIGKILL(和SIGSTOP)。因为这
// 两个信号不能被进程捕获。
	if (signum<1 || signum>32 || signum==SIGKILL)
		return -1;
// 然后根据提供的参数组件sigaction结构内容。sa_handler是指定的信号处理句柄(函数)。
// sa_mask是执行信号处理句柄时的信号屏蔽码。sa_flags是执行时的一些标志组合。这里
// 设定该信号处理句柄只使用1次后就恢复到默认值，并允许信号在自己的处理句柄中受到。
...
// 系统调用的中断处理程序中真正的信号预处理程序(在kernel/system_call.s 119行)。
// 该段代码的主要作用是将信号处理句柄插入到用户程序堆栈中，并在本系统调用结束
// 返回后立刻执行信号句柄程序，然后继续执行用户的程序。这个函数处理比较粗略，
// 尚不能处理进程暂停SIGSTOP等信号。
// 函数的参数是进入系统调用处理程序system_call.s开始，直到调用本函数(system_call.s
// 119行)前逐步压入堆栈的值。这些值包括(在system_call.s中的代码行)：
// 1) CPU执行中断指令压入的用户栈地址ss和esp、标志寄存器eflags和返回地址cs和eip；
// 2) 第83-88行在刚进入system_call时压入栈的寄存器ds、es、fs和edx、ecx、ebx；
// 3) 第95行调用sys_call_table后压入栈中的相应系统调用处理函数的返回值(eax)。
// 4) 第118行压入栈中的当前处理的信号值(signr)。
...
// 如果信号句柄为SIG_IGN(1，默认忽略句柄)则不对信号进行处理而直接返回；如果句柄为
// SIG_DFL(0，默认处理)，则如果信号是SIGCHLD也直接返回，否则终止进程的执行。
// 句柄SIG_IGN被定义为1，SIG_DFL被定义为0。参见include/signal.h，第45、46行。
// 第100行do_exit()的参数是返回码和程序提供的退出状态信息。可作为wait()或waitpid()
// 函数的状态信息。参见sys/wait.h文件第13-18行。wait()或waitpid()利用这些宏就可以
...
// 修改内核态堆栈上用户调用系统调用时的代码指针eip为指向信号处理句柄，同时也将
...
// 是传值函数，因此给eip赋值时需要使用"*(&eip)"的形式。另外，如果允许信号的处理
// 句柄收到信号自己，则也需要将进程的阻塞码压入堆栈。
// 这里请注意，使用如下方式(第104行)对普通C函数参数进行修改是不起作用的。因为当
// 函数返回时堆栈上的参数将会被调用者丢弃。这里之所以可以使用这种方式，是因为该函
// 数是从汇编程序中被调用的，并且在函数返回后汇编程序并没有把调用do_signal()时的
// 所有参数都丢弃。eip等仍然在堆栈中。
// sigaction结构的sa_mask字段给出了在当前信号句柄(信号描述符)程序执行期间应该被
// 屏蔽的信号集。同时，引起本信号句柄执行的信号也会被屏蔽掉。如果允许信号自己的
// 处理句柄程序收到信号自己，则也需要将进程的信号阻塞码压入堆栈。
	*(&eip) = sa_handler;
	longs = (sa->sa_flags & SA_NOMASK)?7:8;
// 将原调用程序的用户堆栈指针向下扩展7(或8)个字长(用来存放调用信号句柄的参数等)，
