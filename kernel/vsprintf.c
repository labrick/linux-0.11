...
// 就为他编写了这段用于内核显示信息的代码。他后来(1998年)承认这段代码中有一个bug，直到
// 1994年才有人发现，并予以纠正。这个bug是在使用*作为输出域宽度时，忘记递增指针跳过这个
// 星号了。在本代码中这个bug仍然存在(130)。他的个人主页是http://liw.iki.fi/liw/
...
// 除操作
// 输入: n为被除数，base为除数；结果: n为商，函数返回值为余数
...
// 下面取得格式指示字符串中的标志域，并将标志常量放入flags变量中。
		...
		repeat:
			...
			switch (*fmt) {
				...
				case '0': flags |= ZEROPAD; goto repeat;    // 填零('0')
			}
		
// 取当前参数字段宽度阈值，放入field_width变量中。如果宽度域中是数值则直接取其为宽度值。
...
// 将字段宽度值取为其绝对值。
		...
		if (is_digit(*fmt))
			...
			field_width = va_arg(args, int);    // 取宽度值？？返回整形？？
			if (field_width < 0) {
...
// 其处理过程与上面宽度域类似。如果精度域中是数值则直接取其为精度值。如果精度域中是
...
// 字段精度值取为0。
		...
		if (*fmt == '.') {
			...
			if (is_digit(*fmt))
				...
				/* it's the next argument */    // 同上也应插入++fmt;
				precision = va_arg(args, int);  // 精度值？？返回整形？？
			}
			if (precision < 0)
				precision = 0;
		}
...
// 放入'宽度域值-1'个空格字符，然后再放入参数字符。如果宽度域还大于0，则表示左靠齐，则在
// 参数字符后面添加'宽度值-1'个空格字符。
		case 'c':
			if (!(flags & LEFT))
				while (--field_width > 0)
					*str++ = ' ';
			*str++ = (unsigned char) va_arg(args, int);     // 返回不是整形了？？
			while (--field_width > 0)
				*str++ = ' ';
			break;
...
// 个空格字符。然后再放入参数字符串。如果宽度域还大于0，则表示为左对齐，则在参数字符串后面
// 添加'宽度值-字符串长度'个空格字符。
		case 's':
			s = va_arg(args, char *);       // args自动有所变化???
			len = strlen(s);
			if (precision < 0)
				precision = len;
