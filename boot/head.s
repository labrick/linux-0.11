...
# 再次注意!!! 这里已经处于32位运行模式，因此这里的$0x10并不是把地址0x10装入各个段寄存器，
# 它现在起始是全局段描述符表中的偏移值，或者更准确地说是一个描述符表项的选择符。
# 有关选择符的说明请参见setup.s中193行下的说明。这里$0x10的含义是请求特权级0(位0-1=0)、
# 选择全局描述符表(位2=0)、选择表中第2项(位3-15=2)。它正好指向表中的数据段描述符项。
# (描述符的具体数值参见前面setup.s中的212，213行)
#
# 下面代码的含义是：设置ds,es,fs,gs为setup.s中构造的数据段(全局段描述符表第2项)的选择
# 符=0x10，并将堆栈放置在stack_start指向的user_stack数组区，然后使用本程序后面定义的新
# 中断描述符表和全局段描述符表。新全局段描述符表中初始内容与setup.s中的基本一样，仅
# 段限长从8MB修改成了16MB。stack_start定义在kernel/sched.c，69行。它是指向user_stack数组
# 末端的一个长指针。第23行设置这里使用的栈，姑且称为系统栈。但在移动到任务0执行
# (init/main.c中137行)以后该栈就被用作任务0和任务1共同使用的用户栈了。

startup_32:				# 18-22行设置各个数据段寄存器
	...
	mov %ax,%fs		# 因为修改gdt，所以需要重新加载所有的段寄存器
	mov %ax,%gs		# CS代码段寄存器已经在setup_gdt中重新加载过了

# 由于段描述符中的段限长从setup.s中的8MB改成了本程序设置的16MB(见setup.s行208-216和
# 本程序后面的235-236行)，因此这里再次对所有段寄存器执行加载操作是必须的。另外，通过
# 使用bochs跟踪观察，如果不对CS再次执行加载，那么在执行到26行时CS代码段不可见部分中的
# 限长还是8MB。这样看来应该重新加载CS。但是由于setup.s中的内核代码段初始化阶段不会有问题，
# 而且在以后内核执行过程中段间跳转时会重新加载CS。因此这里没有加载它并没有让程序出错。
# 针对该问题，目前内核中就在第25行之后添加了一条长跳转指令：'ljmp $(_KERNEL_CS),$lf',
# 跳转到第26行来确保CS确实又被重新加载。

	lss _stack_start,%esp

# 32-36行用于测试A20地址线是否已经开启。采用的方法是向内存地址0x000000处写入任意一个
# 数值，然后看内存0x100000(1M)处是否也是这个数值。如果一直相同的话，就一直比较下去，
# 也即死循环、死机。表示地址A20线没有选通，结果内核就不能使用1MB以上内存。
#
# 33行上的'1:'是一个局部符号构成的标号。标号由符号后跟一个冒号组成。此时该符号表示活动
# 位置计数(Active location counter)的当前值，并可以作为指令的操作数。局部符号用于帮助
# 编译器和编程人员临时使用一些名称。共有10个局部符号名，可在整个程序中重复使用。这些
# 符号名使用名称'0'、'1'、...、'9'来引用。为了定义一个局部符号，需把符号写成'N:'形式
# (其中N表示一个数字)。为了引用先前最近定义的这个符号，需要写成'Nb'，其中N是定义符号时
# 使用的数字。为了引用一个局部标号的下一个定义，需要写成'Nf'，这里N是10个前向引用之一。
# 上面'b'表示“向后(backwards)”，'f'表示“向前(forwards)”。在汇编程序的某一处，我们
# 最大可以向后/向前引用10个标号(最远第10个)。

	xorl %eax,%eax
1:	incl %eax		# check that A20 really IS enabled
	movl %eax,0x000000	# loop forever if it isn't
	cmpl %eax,0x100000
	je 1b				# '1b'表示向后(backward)跳转到标号1去(33行)
 ...
 * 此后"verify_area()"调用就不需要了。486的用户通常也会想将NE(#5)置位，以便对
 *  数学协处理器的出错使用int 16
 */
# 上面原注释中提到的486 CPU中CR0控制寄存器的位16是写保护标志WP(Write-Protect)，用于禁止
# 超级用户级的程序向一般用户只读页面中进行写操作。该标志主要用于操作系统在创建新进程
# 时实现写时复制(copy-on-write)方法。
#
# 下面这段程序(43-65)用于检查数学协处理器芯片是否存在。方法是修改控制寄存器CR0，在假设
# 存在协处理器的情况下执行一个协处理器指令，如果出错的话则说明协处理器芯片不存在，
...
# finit向协处理器发出初始化命令，它会把协处理器置于一个未受以前操作影响的已知状态，设置
# 其控制字为默认值、清楚状态字和所有浮点栈式寄存器。fstsw指令取协处理器的状态字。
# 如果系统中存在协处理器的话，那么在执行了fninit指令后其状态字低字节肯定为0。

check_x87:
	fninit			# 向协处理器发出初始化命令
	fstsw %ax		# 取协处理器状态字到ax寄存器中
	cmpb $0,%al		# 初始化后状态应该为0，否则说明协处理器不存在
	je 1f			/* no coprocessor: have to set bits */
	movl %cr0,%eax	# 如果存在则向前跳转到标号1处，否则改写cr0
	xorl $6,%eax		/* reset MP, set EM */	# 全部置1
	movl %eax,%cr0
	ret

# 下面是一汇编语言指示符。其含义是指存储边界对其调整。"2"表示把随后的代码或数据的偏移
# 位置调整到地址值最后2比特位为0的位置(2^2)，即按4字节方式对其内存地址。不过现在GNU as
# 直接写出对其的值而非2的次方值了。使用该指示符的目的是为了提高32位CPU访问内存中代码
 ...
 * 寄存器(用lidt指令)。真正使用的中断门以后再安装。当我们在其他地方认为一起都正常时再
 * 开启中断。该子程序将会被页表覆盖掉。
 */
# 中断描述符表中的项虽然也是8字节组成，但其格式与全局表中的不同，被称为门描述符(Gate 
# Descriptor)。它的0-1，6-7字节是偏移量，2-3字节是选择符，4-5字节是一些标志。这段
# 代码首先在edx、eax中组合设置出8字节默认的中断描述符值，然后再idt表每一项中都放置该
# 描述符，共256项。eax含有描述符低4字节，edx含有高4字节。内核在随后的初始化过程中会
# 替换安装那些真正实用的中断描述符项。
 ...
 * 该子程序只有两行，“非常的”复杂，所以当然需要这么长的注释了 :-)
 * 该子程序将被页表覆盖掉
 */
setup_gdt:
	lgdt gdt_descr	# 加载全局描述符表寄存器(内容已设置好，见234-238)
	ret
 ...
 */
# 每个页表长为4KB字节(1页内存页面)，而每个页表项需要4个字节，因此一个页表共可以存放1024
# 个表项。如果一个页表项寻址4KB的地址空间，则一个页表就可以寻址4MB的物理内存。
...
.org 0x5000		# 定义下面的内存数据块从偏移地址0x5000处开始
/*
 ...
 */
/*
 * 当DMA(直接存储器访问)不能访问缓冲块时，下面的tmp_floppy_area内存块就可供软盘驱动
 * 程序使用。其地址需要对齐调整，这样就不会跨越64KB边界。
 */
_tmp_floppy_area:
	.fill 1024,1,0	# 共保留1024项，每项1字节，填充数值0

# 下面这几个入栈操作用于为跳转到init/main.c中的main()函数做准备工作。第139行上的指令
# 在栈中压入了返回地址，而第140行则压入了main()函数代码的地址。当head.s最后在第218行
# 执行ret指令时就会弹出main()的地址，并把控制权转移到init/main.c程序中。参见第3章中
# 有关C函数调用机制的说明。
# 前面3个入栈0值应该分别表示envp、argv指针和argc的值，但main()没有用到。
# 139行的入栈操作是模拟调用main.c程序时首先将返回地址入栈的操作，所以如果main.c程序
# 真的退出时，就会返回到这里的标号L6处继续执行下去，也即死循环。
#
# 140行将main.c的地址压入堆栈，这样，在设置分页处理(setup_paging)结束后执行'ret'返回
# 指令时就会将main.c程序的地址弹出堆栈，并去执行main.c程序了。
# 有关C函数调用机制请参见程序后的说明。
after_page_tables:
	pushl $0		# These are the parameters to main :-)
	pushl $0		# 这里是调用main程序的参数(指init/main.c)
...
ignore_int:
	...
	pushl $int_msg	# 把调用printk函数的参数指针(地址)入栈。注意！若符号int_msg
					# 前不加'$'，则表示把int_msg符号处的字长('Unkn')入栈 :-)
	call _printk	# 该函数在kernel/printk.c中
 ...
 *
 * 对于那些有多于16MB内存的家伙 - 真是太幸运了，我还没有，为什么你会有 :-)。代码就
 * 在这里，对它进行修改吧。(实际上，这并不太困难。通常只需修改一些常数等。我把它设置
 * 为16MB，因为我的机器再怎么扩充也不会超过这个界限(当然，我的机器是很便宜的:-))。
 * 我已经通过设置某类标志来给出需要改动的地方(搜索"16MB")，但我不能保证作这些改动
 * 就行了 :-))。
 */
# 上面英文注释第2段的含义是指在机器物理内存中大于1MB的存储空间主要被用来主内存区。
# 主内存区空间由mm模块管理。它涉及到页面映射操作。内核中所有其他函数就是这里指的一般
# (普通)函数。若要使用主内存区的页面，就需要使用get_free_page()等函数获取。因为主内存区
# 中内存页面是共享资源，必须有程序进行统一管理以避免资源争用和竞争。
#
# 在内存物理地址0x0处开始存放1页页目录表和4页页表。页目录表示系统所有进程公用的，而
# 这里的4页表则属于内核专用，它们一一映射线性地址其实16MB空间范围到物理内存上。对于
...
setup_paging:				# 首先对5页内存(1页目录+4页页表)清零
	...
	xorl %edi,%edi			/* pg_dir is at 0x000 */
							# 页目录从0x000地址开始
	cld;rep;stosl			# eax内存存放到es:edi所指内存位置处，且edi增4

# 下面4句设置页目录表中的项，因为我们(内核)共有4个页表所以只需设置4项。
# 页目录项的结构与页表中项的结构一样，4个字节为1项。参见上面113行下的说明。
# 例如"$pg0+7"表示：0x00001007，是页目录表的第1项。
...
# 位置是1023*4 = 4092。因此最后一页是最后一项的位置就是$pg3+4092。
...
# 该返回指令的另一个操作是将140行压入堆栈中的main程序的地址弹出，并跳转到/init/main.c
# 程序去运行。本程序到此就真正结束了。

.align 2			# 按4字节方式对其内存地址边界
.word 0				# 这里先空出2字节，这样224行上的长字是4字节对齐的
...
! 组成一个描述符项，所以表中共可有256项。符号_gdt是全局表在本城中的偏移位置，见234行。
gdt_descr:
	.word 256*8-1		# so does gdt (not that that's any		# 注：not --> note
	.long _gdt		# magic number, but it works for me :^)

	.align 3			# 按8(2^3)字节方式对其内存地址边界
_idt:	.fill 256,8,0		# idt is uninitialized	# 256项，每项8字节，填0

# 全局表。前4项分别是空项(不用)、代码段描述符、数据段描述符、系统调用段描述符，其中系统
# 调用段描述符并没有派用处，Linus当时可能曾想把系统调用代码专门放在这个独立的段中。
# 后面还预留了252项的空间，用于防止所创建任务的局部描述符(LDT)和对应的任务状态段TSS的
# 描述符。
